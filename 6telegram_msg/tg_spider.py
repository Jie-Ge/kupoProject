'''
参考：https://blog.csdn.net/qq_40933467/article/details/113867758
...
# ***********************************************************************************#
# 此处 https://my.telegram.org/auth?to=apps，获取Telegram账号的 api_id 和 api_hash
api_id = 11943814  # your telegram api id
api_hash = '04e4fcdecbce07f3aa1a5001a30c56e9'  # your telegram api hash

# 此处 http://www.liuhuawei.com/archives/384/ 获取你的 bot_token
bot_token = '5359736500:AAGnYuV6s52okXNkifJqmn1kMJ-V1y6BN0o'  # your bot_token

# # get from https://telegram.me/getidsbot
admin_id = 5362964796  # your chat id

save_path = './downloads'  # file save path

# 谷歌网盘信息？
upload_file_set = False  # set upload file to google drive
drive_id = ''  # google teamdrive id 如果使用GD，删除''内的内容即可。
...
# 待下载队列, 队列里的数据结构 [message, chat_title, entity, file_name]
queue = asyncio.Queue()
...
async def handler(update):
    '''
    handler 相当于一个传话员，将你的命令处理过后，生成一个 queue 待下载队列，供 worker 使用;
    通过与 Telegram 客户端的 bot 交互，告诉脚本要爬哪些内容，并依次放入 queue 队列;
    消息格式： /start <群组链接> <从第N条消息开始下载>
    :param update:
    :return:
    '''

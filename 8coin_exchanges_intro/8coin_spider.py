...
class Icon2Thread(threading.Thread):
    ...
    def run(self):
        while not page_queue.empty():
            ...
            # if url_num > 10:
            #     return None
            print(f'正在请求第 {url_num}/22835 个url：', threading.active_count())

            url = f'https://www.mifengcha.com/coindetails/{item["coin_id"]}'
            count = 3  # 重试次数
            while count > 0:
                headers = {
    ...
    def parse_data(self, response, name):
        ...
        tr_tag = page.xpath('//div[@class="main-box bl-tabs"]//div[@class="main"][1]/table[1]/tr[3]')
        if len(tr_tag):
            is_intro = tr_tag[0].xpath('./td[1]/text()')
        else:
            print('请求到的不是正常的detail page')
            fail_page_url.append(response.url)
            return None

        if len(is_intro) and is_intro[0] == '币种简介':
            intro = tr_tag[0].xpath('./td[2]/text()')
            if len(intro):
                desc = intro[0]
        else:
            print('请求到的不是正常的detail page')
...
def main():
    url = 'https://www.mifengcha.com/api/v1/quick_search/coin'
    headers = {
        'user-agent': random.choice(ua.data_browsers['chrome']),
        'referer': 'https://www.mifengcha.com/'
    }
    ...
    if res.status_code == 200:
        ...
        for item in data_json['data']:
            page_queue.put({'coin_id': item['id'], 'name': item['symbol']})
    ...
    with open('coin_desc1.json', 'w', encoding="utf-8") as file_object:
